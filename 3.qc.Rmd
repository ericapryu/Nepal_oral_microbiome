...
plot_genotek_reads <- ggplot(genotek_reads, aes(x= reorder(SampleID, TotalReads), y=TotalReads, fill=Lifestyle)) +
  ...
  geom_hline(yintercept = 26927, linetype="dashed") +
...
# throw out the samples that are below 26927 reads based on read depth
which(genotek_reads == 17743, arr.ind=TRUE) # tells me which row that the sample is in
...
rfxn <- compute_alphadiv(phyloseq_prunetaxa,1000,26927)
```

## generate rarefied phyloseq object subsetted to 26927 sequences
```{r}
raref_phyloseq_prunetaxa <- rarefy_even_depth(phyloseq_prunetaxa, sample.size = 26927, replace = TRUE, rngseed = 123)
```
...
# subset to individuals currently on antibiotics
...
# write.csv(rfxn, file = "output/rarefaction_edit05152023.csv")
```
...
after_count_qiagen <- c(14, 21, 23, 7, 6)
before_count_psoil <- c(14, 22, 23, 8, 4)
after_count_psoil <- c(14, 22, 18, 8, 2)
...
## sample size per extraction kit
```{r}
df_psoil <- after_df_psoil
df_psoil$state <- gsub("After", "Psoil", df_psoil$state)
df_qiagen <- after_df_qiagen
df_qiagen$state <- gsub("After", "Qiagen", df_qiagen$state)

comb <- rbind(df_psoil, df_qiagen)

comb$lifestyle <- gsub("Industrial", "American Industrial", comb$lifestyle)
comb$lifestyle <- factor(comb$lifestyle, levels=c("Foragers", "Recently Settled", "Agriculturalists", "Expats", "American Industrial"))

sample_extraction_kit <- ggplot(comb, aes(fill=state, x=lifestyle, y=count)) + 
  geom_bar(position="dodge", stat="identity") +
    scale_fill_manual(name=NULL,
                    values=twocolors,
                    breaks=c("Psoil", "Qiagen"),
                    labels=c("PowerSoil", "Qiagen")) +
  geom_text(aes(label=count), position=position_dodge(width=0.9), vjust=-0.25) +
  labs(title = "Sample Size per Extraction Kit for each Lifestyle", x = "Lifestyle", y = " Sample Size")+
  theme_classic()

# ggsave(file = "figures/sample_extraction_kit.svg", width = 7, height = 5, plot = sample_extraction_kit)
```
